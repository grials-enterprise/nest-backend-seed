...
- **Request pipeline** composed of a header middleware, a license guard,
...
```
src/
├── app.controller.ts            # Root "Hello World" controller
├── app.module.ts                # Root module (global providers, Mongo, config)
├── app.service.ts
├── main.ts                      # Bootstrap: prefix, versioning, Swagger
├── common/
│   ├── ajv/                     # AJV schemas and validator setup
│   ├── decorators/              # ResponseMessage decorator
│   ├── filters/                 # HttpExceptionFilter
│   ├── guards/                  # LicenseGuard (JWT-based licensing)
│   ├── interceptors/            # Logger + Response interceptors
│   ├── middlewares/             # HeaderMiddleware (header validation/logging)
