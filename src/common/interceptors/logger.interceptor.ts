...
export class LoggerInterceptor implements NestInterceptor {
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    ...
    this.logger.debug(
      `${BALLS.yellow}IN ${method} ${originalUrl} | body: ${requestSize}`,
    );
    ...
    return next.handle().pipe(
      tap({
        next: (data) => {
          ...
          this.logger.debug(
            `${BALLS.green}OUT ${method} ${originalUrl} | ${duration}ms | size: ${responseSize}`,
          );
        },
        error: (err) => {
          const duration = Date.now() - start;
          this.logger.debug(
            `${BALLS.red}ERR ${method} ${originalUrl} | ${duration}ms | ${err.message}`,
          );
        },
      }),
    );
