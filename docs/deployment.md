...
EKS liveness and readiness probes should target the `/health` endpoint. Because
`HeaderMiddleware` validates the required `l-api-version` header on every route
(including `/health`), the probes **must** send that header, otherwise they
receive a `422`.
