...
- Registers global providers:
  - `APP_INTERCEPTOR` → `LoggerInterceptor`, `ResponseInterceptor`
  - `APP_FILTER` → `HttpExceptionFilter`
  - `APP_GUARD` → `LicenseGuard`
- Applies `HeaderMiddleware` to all routes via `configure()`.
...
1. **`HeaderMiddleware`** — validates required headers against the
   `headersValidationData` AJV schema (`l-api-version` is required) and builds
   the initial `logData` object.
